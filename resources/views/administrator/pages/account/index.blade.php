          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item">
                <a href="#" data-bs-toggle="modal" data-bs-target="#create-account" type="submit" class="btn btn-primary"><i class="fas fa-plus"></i></a>
                @include('administrator.pages.account.create')
              </li>              
            </ol>
          </div>
        </div>
                            ...
                            <tbody>
                              ...
                              <tr {{ (!is_null($user->deleted_at) && Auth::user()->role == 1) ? "style=text-decoration:line-through;" : '' }}>
                                  ...
                                  <td class="text-center">
                                    ...
                                    <div class="btn-icon btn" {{ Auth::user()->email == $user->email ? 'hidden' : '' }}>
                                      <ul class="navbar-nav ml-auto">
                                        <li class="nav-item dropdown">
                                          ...
                                          <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">  
                                            ...
                                            @else
                                              <div class="dropdown-divider"></div>
                                              <a data-bs-toggle="modal" data-bs-target="#role-account{{ $user->id }}" type="submit" class="dropdown-item">
                                                <i class="fas fa-tasks mr-2"></i> Phân quyền sử dụng chức năng
                                              </a>
                                              <div class="dropdown-divider"></div>
                                              <a data-bs-toggle="modal" data-bs-target="#notification{{ $user->id }}" type="submit" class="dropdown-item">
                                              ...
                                              <div class="dropdown-divider"></div>
                                              <a data-bs-toggle="modal" data-bs-target="#resetpassword{{ $user->id }}" type="submit" class="dropdown-item">
                                    ...
                                    <!-- popup detail -->
                                    @include('administrator.pages.account.role')
                                    ...
                                    <div class="modal fade" id="resetpassword{{ $user->id }}" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
                                      <div class="modal-dialog">
                                        <div class="modal-content">
                                          <div class="modal-header">
                                            <h5 class="modal-title" id="ModalLabel">Thông tin</h5>
                                          ...
                                          <div class="modal-body">
                                            Bạn muốn chuyển mật khẩu của tài khoản "{{ $user->email }}" về mặc định?
                                          </div>
                                          <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Thoát</button> 
                                              <form action="{{ route('admin.account.reset.password', ['id' => $user->id]) }}" method="POST">
                                                @csrf
                                                <a href="{{ route('admin.account.reset.password', ['id' => $user->id]) }}" type="button" class="btn btn-primary" onclick="event.preventDefault(); this.closest('form').submit()">Đồng ý</a>
                                              </form>
                                          </div>
                                        </div>
                                      </div>

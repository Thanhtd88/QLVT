<div class="modal fade" id="info-ngan-hang" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Danh sách xe đến hạn ngân hàng</h5>
                        ...
                        <div class="card-body body-shadow p-0 border-content-none">
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    @else
                                        @foreach ($ngan_hang_vihicles as $ngan_hang_vihicle)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td class="text-center">{{ $ngan_hang_vihicle->so_xe }}</td>
                                                <td class="text-upper text-center">{{ $ngan_hang_vihicle->nhan_hieu }}</td>
                                                <td class="text-center">{{ $ngan_hang_vihicle->loai_thung }}</td>
                                                <td class="text-center">{{ number_format($ngan_hang_vihicle->khoi_luong_hang_hoa/1000,2) }}</td>
                                                <td class="text-center">{{ date('d-m-Y', strtotime($ngan_hang_vihicle->hieu_luc_ngan_hang)) }}</td>
                                                <td class="text-center">{{ $ngan_hang_vihicle->tg_con_lai }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                </tbody>

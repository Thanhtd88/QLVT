<div class="modal fade" id="info-dang-kiem" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Danh sách xe đến hạn đăng kiểm</h5>
                        ...
                        <div class="card-body body-shadow p-0 border-content-none">
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    @else
                                        @foreach ($dang_kiem_vihicles as $dang_kiem_vihicle)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td class="text-center">{{ $dang_kiem_vihicle->so_xe }}</td>
                                                <td class="text-upper text-center">{{ $dang_kiem_vihicle->nhan_hieu }}</td>
                                                <td class="text-center">{{ $dang_kiem_vihicle->loai_thung }}</td>
                                                <td class="text-center">{{ number_format($dang_kiem_vihicle->khoi_luong_hang_hoa/1000,2) }}</td>
                                                <td class="text-center">{{ date('d-m-Y', strtotime($dang_kiem_vihicle->hieu_luc_kiem_dinh)) }}</td>
                                                <td class="text-center">{{ $dang_kiem_vihicle->tg_con_lai }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                </tbody>

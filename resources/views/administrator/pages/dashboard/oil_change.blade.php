<div class="modal fade" id="info-thay-nhot" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Danh sách xe đến hạn thay nhớt</h5>
                        ...
                        <div class="card-body body-shadow p-0 border-content-none">
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    @else
                                        @foreach ($thay_nhot_vihicles as $thay_nhot_vihicle)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td class="text-center">{{ $thay_nhot_vihicle->so_xe }}</td>
                                                <td class="text-upper text-center">{{ $thay_nhot_vihicle->nhan_hieu }}</td>
                                                <td class="text-center">{{ $thay_nhot_vihicle->loai_thung }}</td>
                                                <td class="text-center">{{ number_format($thay_nhot_vihicle->khoi_luong_hang_hoa/1000,2) }}</td>
                                                <td class="text-center">{{ $thay_nhot_vihicle->quang_duong_con_lai }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                </tbody>
